async function deleteForever(sentinelToken, bearerToken, generationId) {
...
async function main() {
	try {
		...
		const generationIds = JSON.parse(process.argv[4])

		for (const generationId of generationIds) {
			await deleteForever(sentinelToken, bearerToken, generationId)
